...
enum MockChatData {
    ...
    static var pairCount: Int {
        let raw = ProcessInfo.processInfo.environment["USE_MOCK_CHAT_PAIR_COUNT"]?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
    ...
    private static func makeAssistantTurn(pairIndex: Int, rng: inout SplitMix64) -> ChatTurn {
        ...
        case 0:
            ...
            turn.content = markdownRichStatic
                + "\n\n"
        ...
        case 3:
            ...
            turn.toolCalls = [
                ToolCall(
                    id: idA,
                    type: "function",
                    function: ToolCallFunction(name: "grep", arguments: "{\"pattern\": \"func\", \"path\": \"/mock\"}")
                ),
                ToolCall(
                    id: idB,
                    type: "function",
            ...
            turn.toolResults[idA] = "/mock/Sources/App.swift:3:func main()\n/mock/Sources/Util.swift:1:func helper()\n"
        ...
        case 4:
            ...
            turn.toolCalls = [
                ToolCall(
                    id: id,
                    type: "function",
                    function: ToolCallFunction(name: "run_command", arguments: "{\"command\": \"xcodebuild -scheme osaurus test\"}")
                ),
            ]
        ...
        case 5:
            ...
            turn.preflightCapabilities = [
                PreflightCapabilityItem(type: .method, name: "workspace_index", description: "Semantic index over the repo root."),
                PreflightCapabilityItem(type: .tool, name: "read_file", description: "Read UTF-8 text from a path."),
                PreflightCapabilityItem(type: .skill, name: "swiftui_layout", description: "Layout patterns for macOS chat UI."),
            ]
            turn.content = "Preflight loaded the above capabilities; answering with a short plan.\n\n1. profile\n2. fix hotspots\n"
        ...
        case 7:
            ...
            turn.content = "Package manifest uses Swift 6.2; dependencies declared below.\n\n```swift\nlet package = Package(\n    name: \"OsaurusCore\"\n)\n```\n"
        ...
        case 9:
            ...
            turn.toolCalls = [
                ToolCall(
                    id: id,
                    type: "function",
                    function: ToolCallFunction(name: "web_search", arguments: "{\"query\": \"NSTableView diffable data source\"}")
                ),
            ]
            turn.toolResults[id] = "result snippet: Apple docs recommend NSDiffableDataSourceSnapshot for large threads.\n"
            turn.content = ""
        ...
        case 10:
            ...
            turn.content = "> quote: keep frame time under 16ms.\n\n| metric | target |\n| --- | --- |\n| fps | 60 |\n"
    ...
    private static var seedUInt64: UInt64 {
        let raw = ProcessInfo.processInfo.environment["USE_MOCK_CHAT_SEED"]?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
